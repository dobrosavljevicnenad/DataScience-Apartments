...
def load_and_clean_dataset(path):
    ...
    # 3. Rename (opstina → municipality ...)
    df = rename_location_columns(df)

    # 4. Fill missing neighborhood
    df = fill_missing_neighborhood(df)

    # 5. Normalize floor
    df = normalize_floor(df)

    # 6. Impute missing floors
    df = impute_floor(df)

    # 7. Drop street column
    df = drop_street(df)
